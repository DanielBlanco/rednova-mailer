...
case class SesMailer(
  ...
  private def buildEmailRequest(mail: Mail): SendEmailRequest =
    SendEmailRequest(
  ...
  private def asDestination(m: Mail): Destination =
    Destination(
  ...
  private def asEmailAddress(v: String): EmailAddress =
    EmailAddress(v)

  private def asEmailContent(m: Mail): EmailContent =
    EmailContent(
      simple = Message(
  ...
  private def toContent(v: String): Content =
    Content(MessageData(v))

  private def handleError(e: AwsError): MailerException =
